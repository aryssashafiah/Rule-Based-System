...
# DEFAULT RULES FROM THE LAB QUESTION (SCHOLARSHIP CONTEXT)
...
def run_rules(facts: Dict[str, Any], rules: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    best = fired_sorted[0].get(
        "action",
        {"decision": "REVIEW", "reason": "No action specified in rule."},
    )
...
# ----------------------------
st.set_page_config(
    page_title="Scholarship Rule-Based System",
    page_icon="🎓",
    layout="wide",
)
st.title("Scholarship Advisory – Rule-Based System")
st.caption(
    "Enter applicant information, inspect/edit scholarship rules in JSON, "
    "and let the rule engine suggest a decision."
)

with st.sidebar:
    st.header("Applicant Facts")

    cgpa = st.number_input("CGPA", min_value=0.0, max_value=4.0, step=0.01, value=3.50)
    family_income = st.number_input(
        "Monthly family income (MYR)", min_value=0, step=100, value=5000
    )
    co_curricular_score = st.number_input(
        "Co-curricular score (0–100)", min_value=0, max_value=100, step=1, value=75
    )
    community_service_hours = st.number_input(
        "Community service hours", min_value=0, max_value=1000, step=1, value=30
    )
    current_semester = st.number_input(
        "Current semester of study", min_value=1, max_value=12, step=1, value=3
    )
    disciplinary_actions = st.number_input(
        "Number of disciplinary actions", min_value=0, max_value=10, step=1, value=0
    )

    st.divider()
    st.header("Rules (JSON)")
    st.caption(
        "These are the scholarship rules. Start with the default rules given in the lab; "
        "in a real system, the committee could edit them here."
    )
...
# Facts dictionary used by the engine
facts = {
    "cgpa": float(cgpa),
...
if run:
    ...
    with col1:
        ...
        # Handle the different scholarship decisions
